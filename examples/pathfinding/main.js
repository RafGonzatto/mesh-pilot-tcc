...
function onCollisionObstacle(entity, obstacle) {
  logMessage(
    `<span class="highlight">${entity.color.toUpperCase()} colidiu</span> com obstáculo – recalculando rota...`
  );
...
logMessage(
  "NavMesh criado com base nos obstáculos. Rotas bloqueadas não aparecem no grafo."
);
...
class Npc extends Entity {
  ...
  update() {
    ...
    if (
      ...
      this.nextTargetIndex < this.currentPath.length
    ) {
  ...
  _recalcularRota(atualizarGrafo = false) {
    ...
    if (this.currentPolyId < 0 || this.playerPolyId < 0) {
      ...
      this.nextTargetIndex = 0;
      logMessage(
        '<span class="highlight">NPC</span> fora ou Player fora do NavMesh => sem rota.'
      );
      return;
    }

    logMessage(
      `<span class="highlight">NPC BFS</span> de ${this.currentPolyId} → ${this.playerPolyId}`
    );
    const path = Pathfinder.bfs(
      navMesh._graph,
      this.currentPolyId,
      this.playerPolyId,
      true
...
function draw() {
  ...
  // Arestas do grafo
  if (navMesh._graph) {
    for (const node of navMesh._graph.nodes) {
      const cA = node.polygon.getCenter();
      for (const { nodeId } of navMesh._graph.getAdjacencias(node.id)) {
...
logMessage(`
  ...
  2) Se o NPC colidir com um obstáculo, ele recalcula a rota (build_graphConsideringObstacles + BFS).<br>
